...
# If more than this number of locations would be deleted, require
# the --force flag. Allow this number or less locations to be deleted
# without the --force flag. Deletions can happen either because a
# location is gone, or its id has changed.
SANITY_THRESHOLD = 5
...
def main():
    ...
    parser.add_argument(
        "filename", metavar="locations.json", help="locations"
    )
    parser.add_argument(
        "--host", metavar="host[:port]", default='localhost',
    ...
    parser.add_argument(
        "-i", "--index", default=INDEX, help="elasticsearch index"
    )
    parser.add_argument(
        "-t", "--type", default=TYPE, help="elasticsearch document type"
    )
    ...
    try:
        ...
        # index not found, which means there are no old
        # locations to worry about
        old_ids = set()
    ...
    if num_to_delete > SANITY_THRESHOLD:
        logger.warning("deleting %d objects from the database, which is "
                       "larger than the threshold of %d",
                       num_to_delete, SANITY_THRESHOLD)
        if not args.force:
